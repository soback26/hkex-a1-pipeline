...
5. **Phase 4 — Gate 2 diff preview** (⚠️ **mandatory**): Claude shows the projected tracker state, an **F+G robustness summary** counting rows by tag (`[VERIFIED F+G — prospectus]` / `[Single source — prospectus only]` / `[Web cross-checked F+G]` / `[Single source family — verify]` / `[Conflicting — used prospectus]` / `[NOT FOUND — searched: …]`), the full list of new rows, data recoveries, duplicates to delete, and any QC flags, then **waits for your explicit `yes` / `go` / `OK`** before writing anything. Rows tagged `not_found_fg` or `conflicting_fg` block the save until you resolve them with `accept blank <row>` / `keep prospectus <row>` / `keep <secondary> <row>`. Say `dry run only` at any point to exit cleanly without file changes. The robustness tag vocabulary is borrowed from the `/web-research` skill's discipline (Rule 4 independent sources + Rule 10 bounded effort) so a glance at the counter tells you how trustworthy the F+G fields are before they hit the tracker.
